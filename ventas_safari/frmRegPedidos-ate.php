...
<title>Registro de Pedidos - Ate</title>
...
ventana_secundaria = window.open("frmGuiaPedido-ate.php","ventanaB","toolbar=no, scrollbars=no, resizable=no, top=100, left=140, width=1060, height=450");
} 
...
$(document).ready(function(){
          ...
          $.ajax({url:"phpProcess/FillRegPedidosFechas-ate.php",cache:false,type:"GET",data:{des:tde,has:tha},success:function(result){
                $("#tabdet").html(result);
           }});
         });
         })
...
$(document).ready(function(){
          ...
          {        
          $.ajax({url:"phpProcess/FillRegPedidosCliente-ate.php",cache:false,type:"GET",data:{cli:tCl},success:function(result){
                $("#tabdet").html(result);
           }});
          }
         });
 ...
 $(document).ready(function(){
          ...
          {        
          $.ajax({url:"phpProcess/FillRegPedidosSerieNumero-ate.php",cache:false,type:"GET",data:{sp:tSP,ser:tSer,num:tNum},success:function(result){
                $("#tabdet").html(result);
           }});
          }
         });
	...
	<fieldset style="background-color: #A7F5EF">
		...
		<table>
			<tr><td>Documento 
				...
				filtrar_combo("select * from tipos_documento where tpdc_tipodoc='PDA' or tpdc_tipodoc='GFA'");
