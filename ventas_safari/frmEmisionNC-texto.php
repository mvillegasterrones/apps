...
<link href="css/frmEmisionNC.css" media="screen" rel="stylesheet" type="text/css" />
<?php include('libreria.php'); ?>
<?php include('js/Querys.php'); ?>
<html>
<head>
    <?php include 'js/autocompletar.php'; ?>
<title>Emision de Nota de Credito - Texto</title>
...
ventana_secundaria = window.open("frmEmisionDeNC-texto.php","ventanaK","toolbar=no, scrollbars=no, resizable=no, top=100, left=140, width=850, height=445");
} 
...
$(document).ready(function(){
          ...
          $.ajax({url:"phpProcess/FillEmisionNCFechas-texto.php",cache:false,type:"GET",data:{des:tde,has:tha},success:function(result){
                $("#tabdet").html(result);
           }});
         });
         })
...
$(document).ready(function(){
          ...
          {        
          $.ajax({url:"phpProcess/FillEmisionNCCliente-texto.php",cache:false,type:"GET",data:{cli:tCl,tDoc:tTD},success:function(result){
                $("#tabdet").html(result);
           }});
          }
         });
 ...
 $(document).ready(function(){
          ...
          {        
          $.ajax({url:"phpProcess/FillEmisionNCSerieNumero-texto.php",cache:false,type:"GET",data:{sp:tSP,ser:tSer,num:tNum},success:function(result){
                $("#tabdet").html(result);
           }});
          }
         });
...
<table>
    ...
    <td><input type="button" id="btnIr" value="Ir"> &nbsp;
        <td><select name="tpPedido" id="SPed1">
				<?php 
				
				filtrar_combo("select * from tipos_documento where tpdc_tipodoc='NC' or tpdc_tipodoc='ND'");
	...
	<fieldset style="background-color: #A7F5EF">
        <legend style="font-size: 16px">Consulta de Notas de Cr&eacute;dito</legend>
		<table>
			<tr><td>Documento 
				<td><select name="tppedido" id="SPed2">
				<?php 
...
<table border="0" border-color=r"red">
	<tr bgcolor="#242DAE" >
		<th width="130px"><font color="#FAF8F8">Documento
		<th width="140px"><font color="#FAF8F8">Fecha
		<th width="320px"><font color="#FAF8F8">Cliente
