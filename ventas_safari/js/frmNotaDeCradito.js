$(function(){
   ...
   $('#cliente').on('blur','#txtNumero1',function(){
      ...
      if(valor.length >= 7){
         ...
         var consulta = $.ajax({
            type:'POST',
            url:'js/php/frmNotaDeCredito.php',
            data:{txtNumero: valor, txtDocumento:$('#cboDocumento1').val(), txtSerie:$('#txtSerie').val()},
            dataType:'JSON'
         });
         ...
         consulta.done(function(data){
            if(data.error!==undefined){
               ...
               if(data.dato3!==undefined){$('#cliente #txtRUCDNI').val(data.dato3);}
               if(data.dato4!==undefined){$('#cliente #txtDireccion').val(data.dato4);}
               ...
               if(data.dato7!==undefined){$('#cliente #txtMoneda').val(data.dato7);}
               if(data.dato8!==undefined){$('#cliente #txtSaldo').val(data.dato8);}
