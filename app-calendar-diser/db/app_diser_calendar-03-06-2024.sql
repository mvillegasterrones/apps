...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_v1_get_reporte_chart_cargo` (`_idAsistencia` INT)   BEGIN

SELECT 
    DISTINCT(a.seleccione_cargo) AS 'CARGO',
    COUNT(*) AS 'TOTAL_CARGO'
FROM tbl_asistencia a
WHERE a.`idAsistencia` = _idAsistencia
GROUP BY a.seleccione_cargo;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_v1_get_reporte_chart_region` (`_idAsistencia` INT)   BEGIN

SELECT 
    DISTINCT(a.seleccione_dre) AS 'DRE',
    COUNT(*) AS 'TOTAL_CARGO'
FROM tbl_asistencia a
WHERE a.`idAsistencia` = _idAsistencia
GROUP BY a.seleccione_dre;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_v1_save_registro_asistencia` (`_idAsistencia` INT, `_action` VARCHAR(50), `_sel_dre` VARCHAR(80), `_sel_ugel` VARCHAR(100), `_sel_cargo` VARCHAR(100), `_cod_mod` VARCHAR(10), `_dni` VARCHAR(10), `_apellidos_nombres` VARCHAR(100), `_correo` VARCHAR(200), `_celular` CHAR(9))   BEGIN
    IF NOT EXISTS (SELECT * FROM tbl_asistencia a WHERE a.idAsistencia = _idAsistencia AND a.dni = _dni AND a.apellidos_nombres = _apellidos_nombres) THEN
        INSERT INTO tbl_asistencia VALUES (0, _idAsistencia, _action, _sel_dre, _sel_ugel, _sel_cargo, _cod_mod, _dni, _apellidos_nombres, _correo, _celular);
        END IF;
END$$
...
CREATE TABLE `tbl_asistencia` (
  ...
  `seleccione_dre` varchar(80) DEFAULT NULL,
  `seleccione_ugel` varchar(100) DEFAULT NULL,
  `seleccione_cargo` varchar(100) DEFAULT NULL,
...
INSERT INTO `tbl_asistencia` (`id`, `idAsistencia`, `action`, `seleccione_dre`, `seleccione_ugel`, `seleccione_cargo`, `cod_mod`, `dni`, `apellidos_nombres`, `correo`, `celular`) VALUES
(1, 3, 'save-registro-asistencia', 'DRE AMAZONAS', 'UGEL INTERCULTURAL BILINGÜE RIO SANTIAGO', '* Especialistas MINEDU', '0000000', '47352403', 'VILLEGAS TERRONES MANUEL ANTONIO', 'MVILLEGAST@minedu.gob.pe', '932449843'),
(2, 3, 'save-registro-asistencia', 'DRE CAJAMARCA', 'UGEL CONTUMAZÁ', '* Docentes', '0000000', '41095420', 'RODRIGUEZ FERNANDEZ SONIA LILY', 'SRODRIGUEZ@minedu.gob.pe', '-'),
(3, 2, 'save-registro-asistencia', 'DRE ANCASH', 'UGEL ANTONIO RAYMONDI', '* Coordinador/a del Núcleo Educativo', '0000000', '47352403', 'VILLEGAS TERRONES MANUEL ANTONIO', 'MVILLEGAST@minedu.gob.pe', '-'),
(4, 3, 'save-registro-asistencia', 'DRE AMAZONAS', 'UGEL IBIR-IMAZA', '* Docentes', '0000000', '47352402', 'VENEGAS VIVANCO WILSON', 'VENEGAS@gmail.com', '998899888');
...
--
ALTER TABLE `tbl_asistencia`
  ADD CONSTRAINT `tbl_asistencia_ibfk_1` FOREIGN KEY (`idAsistencia`) REFERENCES `tbl_calendar` (`id`);
